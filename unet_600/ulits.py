...
class Units(object):
    def __init__(self, net: torch.nn.Module, net_name, EPOCH, train_data, test_data,
        ...
        if mdl_path is not None:
            self.net.load_state_dict(torch.load(mdl_path))
    ...
    def run(self, colour_train='yellow', colour_test='blue'):
        ...
        with open(f'./LOG/{self.net_name}.txt', 'w') as f1:
            f1.write(f'Epoch:\tlr\ttrain_loss:\ttest_loss\n')
            for epoch in range(self.epoch):
                self.net.train()
                t_loss = self.train(epoch, self.train_data, colour=colour_train)
                self.scheduler.step()
                self.net.eval()
                v_loss = self.test(epoch, self.test_data, colour_test)
                paths = os.path.join(f"./Mdl_Data/{self.net_name}", f"{epoch}.pth")
                torch.save(self.net.state_dict(), paths)
                lr = "{:g}".format(self.scheduler.get_last_lr()[0] * 100)
                f1.write(f'{epoch}\t{lr}\t{t_loss}\t{v_loss}\n')
    ...
    def ConfusionMatrix(slef, numClass, imgPredict, Label):
        ...
        label = numClass * Label[mask].astype(np.int64) + imgPredict[mask]
